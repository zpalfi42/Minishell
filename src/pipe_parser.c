...
/*   Updated: 2022/09/23 12:43:18 by zpalfi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	pipe_parser_2(t_data *data, int i)
{
	...
	while (data->pipes > 0)
	{
		...
		while (data->tokens[++i] != 0)
			if (!ft_strncmp(data->tokens[i], "|\0", 2))
				break ;
		if (data->tokens[i] != 0 && !ft_strncmp(data->tokens[i + 1], "|\0", 2))
		{
...
int	pipe_parser(t_data *data)
{
	...
	if (i == 0 || (data->tokens[i] != 0
			&& !ft_strncmp(data->tokens[i + 1], "|\0", 2)))
	{
		printf("zsh: parse error near '%s'\n", data->tokens[i]);
		return (-1);
	}
